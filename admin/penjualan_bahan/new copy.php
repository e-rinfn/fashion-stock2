    ...
    if (count($bahanIds) !== count(array_unique($bahanIds))) {
        ...
        if (!isset($error)) {
            ...
            foreach ($items as $item) {
                ...
                $bahan = query("SELECT harga_per_satuan FROM bahan_baku WHERE id_bahan = $id_bahan")[0];
                $total_harga += $bahan['harga_per_satuan'] * $qty;
            }
            ...
            if (!isset($error)) {
                ...
                try {
                    ...
                    foreach ($items as $item) {
                        $id_bahan = intval($item['id_bahan']);
                        $qty = intval($item['qty']);
                        $bahan = query("SELECT harga_per_satuan, jumlah_stok FROM bahan_baku WHERE id_bahan = $id_bahan")[0];
                        ...
                        $sql_detail = "INSERT INTO detail_penjualan_bahan (id_penjualan_bahan, id_bahan, jumlah, harga_satuan, subtotal) 
                                       VALUES ($id_penjualan_bahan, $id_bahan, $qty, {$bahan['harga_per_satuan']}, $subtotal)";
                    ...
                    header("Location: detail.php?id=$id_penjualan_bahan");
                    exit();
                } catch (Exception $e) {
                    $conn->rollback();
                    $conn->autocommit(TRUE);
...
<body>
                                    ...
                                    <form method="post" id="formPenjualanBahan">
                                        <div class="card border border-dark shadow-sm rounded-3">
                                            <!-- <div class="card-header">
                                                <h3>Informasi Penjualan Bahan</h3>
                                            </div> -->
                                            <div class="card-body">
                                                <?php if (isset($error)): ?>
    ...
    <script>
        ...
        document.getElementById('tambahBahan').addEventListener('click', function() {
            ...
            //     alert('Semua bahan sudah ditambahkan atau tidak ada stok tersedia');
            //     return;
            // }
        ...
        function initRowEvents(rowId) {
            ...
            select.addEventListener('change', function() {
                ...
                // Hapus bahan sebelumnya dari daftar yang dipilih
                if (previousBahanId) {
                ...
                // Jika bahan dipilih (bukan kosong)
                if (newBahanId) {
                    // Tambahkan ke daftar bahan yang dipilih
                    selectedBahan.push(newBahanId);
                    select.dataset.previousValue = newBahanId;

                    const bahan = bahanData.find(p => p.id_bahan == newBahanId);
                    if (bahan) {
                        row.querySelector('.harga').textContent = formatCurrency(bahan.harga_per_satuan);
        ...
        function updateBahanDropdowns() {
            document.querySelectorAll('.select-bahan').forEach(select => {
                ...
                availableBahan.forEach(bahan => {
                    ...
                    options += `<option value="${bahan.id_bahan}" ${selected}>${bahan.nama_bahan}</option>`;
                });
