    ...
    <script>
        ...
        document.getElementById('tambahBahan').addEventListener('click', function() {
            ...
            const availableBahans = bahanData.filter(bahan => {
                return !selectedBahans.includes(bahan.id_bahan) && bahan.jumlah_stok > 0;
            });
            ...
            availableBahans.forEach(bahan => {
                options += `<option value="${bahan.id_bahan}" data-harga="${bahan.harga_per_satuan}">
                                ${bahan.nama_bahan}
                            </option>`;
            });
        ...
        function initRowEvents(rowId) {
            ...
            select.addEventListener('change', function() {
                ...
                if (previousBahanId) {
                    selectedBahans = selectedBahans.filter(id => id != previousBahanId);
                }
                ...
                if (newBahanId) {
                    selectedBahans.push(newBahanId);
                    select.dataset.previousValue = newBahanId;

                    const bahan = bahanData.find(b => b.id_bahan == newBahanId);
                    if (bahan) {
        ...
        function updateBahanDropdowns() {
            document.querySelectorAll('.select-bahan').forEach(select => {
                ...
                availableBahans.forEach(bahan => {
                    ...
                    options += `<option value="${bahan.id_bahan}" data-harga="${bahan.harga_per_satuan}" ${selected}>
                                    ${bahan.nama_bahan}
                                </option>`;
                });
